<script>
	...
	export let errors = { taskName: "", taskDate: "", categorySelected: "" };
	...
	function validate(name, date, categorySelected) {
	  errorFound = false;
	  let tempErrors = { taskName: "", taskDate: "", categorySelected: "" };
	  ...
	  if (errors.taskName || errors.taskDate || errors.categorySelected) {
		errorFound = true;
	  }
	}
	...
	function addTask(event) {
	  ...
	  validate(taskName, taskDate, categorySelected);
  
	  if (!errorFound) {
		saveTask(taskName, taskDate, categorySelected);
	  } else {
		console.log("Des erreurs ont été trouvées, la tâche n'a pas été ajoutée");
	  }
	}
  
	function saveTask(name, date, category) {
	  const task = { name, date, category };
  ...
  <section>
	...
	<form on:submit={addTask}>
	  ...
	  <input
		...
		id="taskDate"
		bind:value={taskDate}
		on:change={() => validate(null, taskDate, null)}
	  />
	  {#if errors.taskDate}
